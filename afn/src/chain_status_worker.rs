...
pub trait ChainStatusUpdater {
    ...
    fn update(&self, status: &ChainStatus) -> Result<ChainStatus>;
}
    ...
    fn update(&self, status: &ChainStatus) -> Result<ChainStatus> {
        match status {
            ...
            ChainStatus::Good { tail } => {
                let start = Instant::now();
                let mut new_tail = VecDeque::<Block>::new();
                let [finalized, latest] = get_blocks_that_must_exist(
                    self.rpc.as_ref(),
        ...
        let handle = ctx.spawn(worker_name, {
            ...
            move |ctx, worker_name| -> Result<()> {
                ...
                ctx.repeat(poll_interval, move |_ctx| {
                    match chain_status_updater.update(&chain_status) {
                        Ok(new_chain_status) => {
